...
// Autor(a): Karla Cornejo y Lizardo Castillo
...
public:
    ...
    // Autor(a): Karla Cornejo y Lizardo Castillo
    void imprimirGraphviz(const std::string& filename) const {
        std::ofstream file(filename);
        file << "digraph G {\n";
        if (raiz) {
            imprimirGraphvizInterno(file, raiz);
        }
        file << "}\n";
        file.close();
    }
...
private:
    ...
    // Autor(a): Karla Cornejo y Lizardo Castillo
    void imprimirGraphvizInterno(std::ofstream& file, Nodo* nodo) const {
        static int nullCount = 0;

        if (nodo->esHoja) {
            file << "node" << nodo << " [label=\"";
            ...
            if (nodo->siguiente) {
                file << "node" << nodo << " -> node" << nodo->siguiente << " [color=blue];\n";
            }
        } else {
            file << "node" << nodo << " [label=\"";
            for (size_t i = 0; i < nodo->llaves.size(); ++i) {
                file << "<f" << i << "> " << nodo->llaves[i] << " |";
            }
            file << "<f" << nodo->llaves.size() << "> \", shape=record];\n";

            for (size_t i = 0; i < nodo->hijos.size(); ++i) {
                file << "node" << nodo << ":f" << i << " -> node" << nodo->hijos[i] << ";\n";
                imprimirGraphvizInterno(file, nodo->hijos[i]);
            }
        }
    }
    // Autor(a): Karla Cornejo y Lizardo Castillo
...
private:
    ...
    int pistaDisponible;
    int sectorDisponible;
    int bloqueDisponible;
    std::string direccionSector;
    std::string direccionBloque;
    /*Maximos*/
    int espacioSectorMax;
    int espacioBloqueMax;
    int espacioRegistro;
    /*Disponibles Resta*/
    int espacioDisponibleSector;
...
public:
    ...
    Disco(int nroPlato, int nroSuperficie, int nroPistaxSuperficie, int nroSectorxPista, int byteSector, int cantBloque) {
        ...
        bloqueDisponible = 1;
        //Maximos
        this->espacioSectorMax = this->byteSector;
        this->espacioBloqueMax = this->capBloque;
        this->espacioRegistro = 50;
        this->espacioDisponibleSector = espacioSectorMax;
        this->espacioDisponibleBloque = espacioBloqueMax;
    }
    // Funciones GET --- Autor(a): Karla Cornejo y Lizardo Castillo
    ...
    int getPlatoDisponible() { return platoDisponible; }
    int getSuperficieDisponible() { return superficieDisponible; }
    int getPistaDisponible() { return pistaDisponible; }
    int getSectorDisponible() { return sectorDisponible; }
    int getBloqueDisponible() { return bloqueDisponible; }

    int getEspacioDisponibleSector() { return espacioDisponibleSector; }
    int getEspacioDisponibleBloque() { return espacioDisponibleBloque; }

    std::string getDireccionSector() { return direccionSector; }
    std::string getDireccionBloque() { return direccionBloque; }

    // Autor(a): Lizardo Castillo
    bool actualizarEspacioDisponible() {
        if (espacioDisponibleSector < espacioRegistro) {
            sectorDisponible++;
            espacioDisponibleSector = espacioSectorMax;
        }

        if (espacioDisponibleBloque < espacioRegistro) {
            bloqueDisponible++;
            espacioDisponibleBloque = espacioBloqueMax;
        }

        if (sectorDisponible > nroSectorxPista) {
            pistaDisponible++;
            sectorDisponible = 1;
        }

        if (pistaDisponible > nroPistaxSuperficie) {
            superficieDisponible++;
            pistaDisponible = 1;
        }

        if (superficieDisponible > nroSuperficie) {
            platoDisponible++;
            superficieDisponible = 1;
        }

        if (platoDisponible > nroPlato) {
            platoDisponible = 1;
            superficieDisponible = 1;
        }

        if (bloqueDisponible > cantBloque) {
            std::cerr << "No hay espacio disponible en el disco." << std::endl;
            return false;
        }
        direccionSector = "D#PL" + std::to_string(platoDisponible) + "#SU" + std::to_string(superficieDisponible) + "#PI" + std::to_string(pistaDisponible) + "#SE" + std::to_string(sectorDisponible);
        direccionBloque = "disco/bloques/" + std::to_string(bloqueDisponible) + ".txt";
...
public:
    ...
    void agregarInformacion(const std::string& informacion) {
        ...
        //std::cout << "Almacenamiento usado " << almacenamientoUsado << std::endl;
        //archivo.close();
    }
    // Autor(a): Karla Cornejo
...
public:
                        ...
                        switch (opcion) {
                            case 1:
                                ...
                                do {
                                    ...
                                    std::cout << "(4) Buscar Datos Menor de ..." << std::endl;
                                    std::cout << "(0) Salir" << std::endl;
                                    std::cout << "Opcion: ";
                                    std::cin >> opcionDatos;
                                    switch (opcionDatos) {
                                        case 1: {
                                            if (disco.actualizarEspacioDisponible()) {
                                                int idBusqueda;
                                                std::cout << "ID: ";
                                                std::cin >> idBusqueda;
                                                ...
                                                if (comprobar) {
                                                    ...
                                                    pageID = disco.getBloqueDisponible();
                                                    ...
                                                    bufferPool.ActualizarTabla();
                                                    char accion = 'e';
                                                    ManejarAccionPagina(archivoBloque, frame, pageID, contenido, accion, arbolBPlus, userID, idBusqueda, disco);
                                                }
                                                break;
                                            } else {
                                                std::cout << "Espacio insuficiente" << std::endl;
                                                break;
                                            }
                            ...
                            case 8:
                                arbolBPlus.imprimir();
                                arbolBPlus.imprimirGraphviz("arbol.dot");
                                break;
                            case 0:
                                break;
                            default:
    ...
    void insertarDatosDesdeCSV2(ArbolBPlus& arbolBPlus, BufferPool::Frame* frame, Disco& disco) {
        ...
        while (std::getline(archivoCSV, line)) {
            frame = BuscarFramePorPagina(disco.getBloqueDisponible());
            if (!frame) {
                frame = BuscarFrameVacio();
                if (!frame) {
                    if (usarLRU) {
                        LRU(disco.getBloqueDisponible());
                    } else {
                        AlgoritmoClock(disco.getBloqueDisponible());
                    }
                    frame = BuscarFramePorPagina(disco.getBloqueDisponible());
                } else {
                    frame->pageID = disco.getBloqueDisponible();
                    frame->pinCount = 0;
                    frame->dirtyBit = false;
                    bufferPool.ActualizarTabla();
                }
            ...
            frame->pinCount++;
            if (usarLRU) {
                ActualizarLRU(disco.getBloqueDisponible());
            } else {
                frame->refBit = true;
            }
            bufferPool.ActualizarTabla();
            ...
            if(disco.actualizarEspacioDisponible()) {
                direccion = disco.getDireccionSector();
                ...
                if (archivoIndice.is_open()) {
                    archivoIndice << disco.getBloqueDisponible() << std::endl;
                    archivoIndice.close();
                } else {
                ...
                std::string rutaArchivo = disco.getDireccionBloque();
                ...
                if (archivoEntrada.is_open()) {
                    ...
                    while (std::getline(archivoEntrada, linea)) {
                        ...
                        if (!encontrada && linea.find(direccion) != std::string::npos) {
                            ...
                            encontrada = true;
                            disco.espacioDisponibleSector -= disco.espacioRegistro;
                            disco.espacioDisponibleBloque -= disco.espacioRegistro;
                        }
                    }
                    ...
                    if (archivoSalida.is_open()) {
                        ...
                        archivoSalida.close();
                        //std::cout << "Texto agregado correctamente al archivo " << disco.getBloqueDisponible() << ".txt." << std::endl;
                        ...
                        std::cerr << "No se pudo abrir el archivo " << disco.getBloqueDisponible() << ".txt para escritura." << std::endl;
                    }

                } else {
                    std::cerr << "No se pudo abrir el archivo " << disco.getBloqueDisponible() << ".txt para lectura." << std::endl;
                }
                ...
                // Actualizar el frame
                frame->pinCount--;
                ActualizarDirtyBit(disco.getBloqueDisponible());
                bufferPool.ActualizarTabla();
            } else {
                std::cerr << "No hay espacio disponible en el disco." << std::endl;
                break;
        ...
        if (accion == 'l') {
            ...
            while (std::getline(archivoBloque, linea)) {
                ...
                if (std::getline(iss, token, ' ')) {
                    ...
                    if (token.rfind("D#", 0) == 0) { // Verificar si la línea comienza con "D#"
                        direccion = token;
                    }
                }
            }

            archivoBloque.clear(); // Limpiar el estado del flujo
            archivoBloque.seekg(0, std::ios::beg); // Volver al principio del archivo
    ...
    // Autor(a): Karla Cornejo y Lizardo Castillo
    void ManejarAccionPagina(std::ifstream& archivoBloque, BufferPool::Frame* frame, int pageID, std::string& contenido, char& accion, ArbolBPlus& arbolBPlus, int userId, int idBusqueda, Disco& disco) {
        std::string respuesta;
        while (true) {
            if (accion == 'l') {
                std::string line;
                ...
                if (longitudFija("esquemaF.txt", *frame, bufferPool, arbolBPlus, userId, idBusqueda, disco, pageID)) {
    ...
    // Autor(a): Karla Cornejo
    bool longitudFija(const std::string& esquemaArchivo, BufferPool::Frame& frame, BufferPool& bufferPool, ArbolBPlus& arbolBPlus, int userId, int idBusqueda, Disco& disco, int pageID) {
        int userID = userId;
        // Leer el esquema desde el archivo
        ...
        if (respuesta == "si") {
            ...
            direccion = std::to_string(pageID);
            ...
            // Ruta del archivo 1.txt
                ...
                std::string palabraClave = disco.getDireccionSector();
                ...
                if (archivoEntrada.is_open()) {
                    ...
                    if (archivoSalida.is_open()) {
                        ...
                        if (archivoDestino.is_open()) {
                            ...
                            archivoDestino.close();
                            std::cout << "Texto agregado correctamente" << std::endl;
                        } else {
                            std::cerr << "No se pudo abrir el archivo destino para escritura." << std::endl;
                        }
